...
class load_profile {
  ...
  double wind_velocity_50 = B + 1.0 / A * (-log(-NS * log(1.0 - p_e + p_e * 49/50)));
...
public:
  ...
  
  {
    ...
    // T_s be the time between occurrences of wind segment
    T_s[0] = exp_Ts(generator);	
    // the load is 0 in that occurrence time
    load_s[0] = 0;
    
    for(int i = 1; i < N; ++i){
      
      if(i % 2 == 0){
        // it is the time between occurrences
        ...
        // generate the time
        T_s[i] = T_s[i-1] + exp_Tp(generator);				  
        if(r_n > p_e){
          // there is no wind in that segment
          load_s[i] = 0;
        }
        else{
  ...
  double operator() (double t){
    ...
    double load_t = phi * R0 * (gamma * D_d + D_s)/(gamma * alpha_d + alpha_l);
    return load_t;
  }
...
#pragma omp parallel for schedule(dynamic) private(t, x, pars)  
    ...
    prob_name << "USprobwind_" << load_profile_par["phi"] << ".csv";
    prob_noDOL_name << "USprobwind_noDOL_" << load_profile_par["phi"] << ".csv";
    time_name << "UStimewind_" << load_profile_par["phi"] << ".csv";
