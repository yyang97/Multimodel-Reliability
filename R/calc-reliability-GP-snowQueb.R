...
for (ii in 1:nrep) {
  ...
  tau <- function(t) {
    # express t in years
    phi*R_0*(gamma*D_d[[ii]] + D_e[[ii]](t))/(gamma*alpha_d + alpha_l)
  }
...
save(failmat, file = paste0("GPprobSnowQueb_", phi, ".rda"))
...
# 
# 
# PF_post <- rowMeans(failmat)
# quantile(PF_post,probs = c(0.025,.5,0.975))
# 
# 
# quantile(probVan_2[,1],probs = c(0.025,.5,0.975))
# quantile(USprobVan_2[,1],probs = c(0.025,.5,0.975))
# 
# 
# 
# quantile(probQueb_2[,1],probs = c(0.025,.5,0.975))
# quantile(USprobQueb_2[,1],probs = c(0.025,.5,0.975))



#quantile(probwind_2[,1],probs = c(.025,0.5,.975))
#2.5%    50%  97.5% 
#0.2170 0.2835 0.3810 